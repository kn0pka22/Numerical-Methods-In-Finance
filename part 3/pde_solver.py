...
def solve_1d_pde(scheme: FiniteDifferencingScheme, lower_boundary_condition: Callable[[float, float], float], upper_boundary_condition: Callable[[float, float], float],
                terminal_condition: Callable[[np.ndarray], float], pde_coefficients: Callable[[float], dict], number_of_spatial_levels: int, number_of_time_steps: int,
                x_min: float, x_max: float, t_start: float, t_end: float, 
                strike: float = None, spot: float = None, interest_rate: float = None, volatility: float = None) -> tuple[Dict[str, np.ndarray], np.ndarray]:
    """PDE solver.
    ...
    Args:
        ...
        strike (float): strike price for American option.
        spot (float): initial spot price for American option.
        interest_rate (float): risk-free rate for American option.
        volatility (float): volatility for American option.
    """
    ...
    match scheme:
        ...
        case FiniteDifferencingScheme.AmericanPut:
            if strike is None or spot is None or interest_rate is None:
                raise ValueError("Для AmericanPut необходимо передать параметры strike, spot и interest_rate")
            ...
            # Main propagation loop, evaluating u^{m} = A * u^{m+1}.
            for i, t in enumerate(reversed(time_grid[:-1])):
                # current_tau = t #t_end - t
                ...
                result_slice[-1] = upper_boundary_condition(
                    spatial_grid[-1], t)
                # Adjusting the second element u^{m}_{1} in work space slice to satisfy boundary conditions.
                workspace_slice[1] -= lower_diagonal[1] * result_slice[0]
                # Adjusting the penultimate element u^{m}_{N-2} in work space slice to satisfy boundary conditions.
                workspace_slice[-2] -= upper_diagonal[-2] * result_slice[-1]
                # Solve u^{m} = A * u^{m+1} and retrieve u^{m+1}.
                result_slice[1:-1] = inv_propagator.solve(workspace_slice[1:-1])
                
                # PV at t
                S = spot * np.exp(spatial_grid[1:-1] - (interest_rate - volatility**2 / 2.0) * (t_end - t))
                intrinsic_values = np.maximum((strike - S) * np.exp(interest_rate * (t_end - t)), 0)
                
                result_slice[1:-1] = np.maximum(result_slice[1:-1], intrinsic_values)
